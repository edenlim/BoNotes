...
// Define the data we expect from Laravel
...
// Component State
const state = ref(null); // 'like', 'dislike', or null
...
const toggleLike = () => {
    if (state.value === 'like') {
        ...
        if (state.value === 'dislike') dislikes.value--;
        state.value = 'like';
        likes.value++;
    }
};
...
const toggleDislike = () => {
    if (state.value === 'dislike') {
        state.value = null;
        dislikes.value--;
    } else {
        if (state.value === 'like') likes.value--;
        state.value = 'dislike';
        dislikes.value++;
    }
};
...
<template>
    <div class="flex gap-4 select-none m-[0.5rem]">
        ...
        <div @click.stop="toggleLike" class="inline-flex items-center gap-1 cursor-pointer">
            ...
            <p :class="state === 'like' ? 'text-like-green-active' : 'text-like-green'">
                {{ likes }}
            </p>
        </div>

        <!-- Dislike Button (Rotated SVG for Dislike) -->
        <div @click.stop="toggleDislike" class="inline-flex items-center gap-1 cursor-pointer">
            ...
            <p :class="state === 'dislike' ? 'text-dislike-red-active' : 'text-dislike-red'">
                {{ dislikes }}
            </p>
        </div>
    </div>
